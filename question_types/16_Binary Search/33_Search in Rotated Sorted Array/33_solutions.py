    ...
    def search(self, nums: List[int], target: int) -> int:
        ...
        while begin<end:
            ...
            if nums[mid]==target:
                ...
                if nums[mid]<target and target<=nums[end-1]:#说明此次循环mid和end呈单调状态，找到单调子区间，且target在此子区间内（也在右区间）,end虽然是开区间，但是nums[end]会index out of range
                    begin=mid+1 #就可以把begin挤到右区间来了，可以开始用常规二分
                else:
                    end=mid#target掉进不单调的左半了，砍掉没有target的那半,但是此时还不能二分因为不单调
